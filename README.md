# socal pug chatbot
Latest changes:
- Declutter: Reduce the number of times the line is automatically displayed after certain commands
- Detect spam & prevent simultaneous execution of commands.  This should stop the bot from crashing from now on
- !skip now completely ends the current !next process to prevent unnecessary messages & allow for faster line management

TODO:
- More admin commands (rearrange the line, add anyone to any spot in the line, remove anyone from any spot in the line)
